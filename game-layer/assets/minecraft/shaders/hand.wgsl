...
@group(2) @binding(0) var<storage, read> matrices: array<mat4x4<f32>>;

@vertex
fn vs_main(model: VertexInput) -> VertexOutput {
    var output: VertexOutput;

    // Распаковываем локальные координаты куба (0 или 1)
    let local_x = f32(model.packed_data & 0x3Fu);
    let local_y = f32((model.packed_data >> 6u) & 0x3Fu);
    let local_z = f32((model.packed_data >> 12u) & 0x3Fu);
    
    let side_id = (model.packed_data >> 18u) & 0x7u;
    let block_type_id = (model.packed_data >> 23u) & 0xFFFu;
    let material_id = get_final_texture_layer(block_type_id, side_id);

    output.material_id = material_id;

    let model_matrix = matrices[model.instance_idx];
    output.clip_position = proj * model_matrix * vec4<f32>(local_x, local_y, local_z, 1.0);

    // Считаем UV по локальным координатам, чтобы трипланарка не плыла
    if (side_id == SIDE_TOP || side_id == SIDE_BOTTOM) {
        output.uv = vec2<f32>(local_x, local_z);
    } else if (side_id == SIDE_NORTH || side_id == SIDE_SOUTH) {
        output.uv = vec2<f32>(1.0 - local_x, 1.0 - local_y);
    } else {
        output.uv = vec2<f32>(local_z, 1.0 - local_y);
    }
...
fn get_final_texture_layer(block_type: u32, side_id: u32) -> u32 {
    ...
    // Вычисляем индекс в плоском массиве без умножения на CPU: 
    // вместо `profile_id * 6` компилятор сделает быстрый битовый сдвиг и сложение
